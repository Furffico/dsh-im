...
| Discord | 使用 Developer Portal 生成的 Bot Token | Gateway v10 长连接；私信直接回复；服务器文字/公告频道首次 @ 后创建原生 Thread，后续在线程中无需重复 @，并通过编辑消息流式显示回答。设置页可配置新建会话的默认 permission（跟随 Host 默认、`workspace-write` 或 `danger-full-access`） |
...
模型调用文件回传工具后，插件把指定文件交给当前渠道的原生接口。`path` 可以是单个字符串，也可以是字符串数组；一次数组调用会登记多个文件。Discord 会尽量把同一次调用的文件合并成一条附件消息（每条最多 10 个附件，超出后继续分条），其他渠道仍按登记顺序逐个发送。图片会优先以原生图片消息呈现；渠道不支持或明确拒绝图片发送时自动回退为文件附件，发送结果不确定时不会补发文件造成重复消息。插件不额外设置文件来源、创建时间、工作区边界、扩展名、内容、数量、大小或有效期规则；文件只需真实存在且可读取。渠道平台仍可能依据自身权限、配额、文件能力或账号等级拒绝发送，插件会按平台返回结果提示。
